import type { V2_MetaFunction, useLoaderData } from "@remix-run/node";
...
export const meta: V2_MetaFunction = () => [{ title: "Randalls | Home" }];
...
export const loader = async ({ request, params }: LoaderArgs) => {
  ...
  return { projects, services, testimonies, blog }
};
...
export default function Home() {
  const { services, testimonies, blogs, projects } = useLoaderData<typeof loader>()
  const serviceSlides = services.map((service) => <ServiceSlide service={service} />)
  const testimonySlides = testimonies.map((testimony) => <TestimonySlide testimony={testimony} />)
  const blogSlides = blogs.map((blog) => <BlogSlide blog={blog} />)
  const projectSlides = projects.map((project) => <ProjectSlide project={project} />)
  return (
    <main>
      {/* hero area start */}
